...
window.onload=function(){
  ...
  for(var i=0;i<[lines];i++){
    ...
    document.getElementsByClassName("chn")[i].childNodes[1].innerHTML=namec[i].childNodes[0].nodeValue.replace(/(#1|#2|#3|##)/g,function($0, $1){return{"#1":'<span class="cgb">',"#2":'<span class="cgd">',"#3":"</span>","##":"<i>未使用</i>"}[$1]});
    ...
    if(rmk==1){
      document.getElementsByClassName("chn")[i].childNodes[3].innerHTML="<p>"+remkc[i].childNodes[0].nodeValue.replace(/\n/g,"</p><p>").replace(/(#1|#2|#3)/g,function($0, $1){return{"#1":'<span class="cgb">',"#2":'<span class="cgd">',"#3":"</span>"}[$1]})+"</p>";
  ...
  document.getElementsByClassName('content')[1].onclick=function(event){if(event.target.className==='top'){document.getElementsByClassName('content')[1].insertBefore(event.target.parentNode,event.target.parentNode.parentNode.firstChild)}}
}